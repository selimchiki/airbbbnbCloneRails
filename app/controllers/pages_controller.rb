class PagesController < ApplicationController
  ...
  def search
    if params[:search].present? && params[:search].strip != ""
      session[:airbbbnb_search] = params[:search]
    end

    arrResult = Array.new

    if session[:airbbbnb_search] && session[:airbbbnb_search] != ""
      @rooms_address = Room.where(active: true).near(session[:airbbbnb_search], 5, order:'distance')
    else
